...
final class DetailModel extends KompyAbsModel // DibiRow obstará korektní načtení dat
{
	...
	public function getId($order = NULL, $where = NULL, $offset = NULL, $limit = NULL){
		$result =  dibi::query(
	...
	public function getDefComp($order = NULL, $where = NULL, $offset = NULL, $limit = NULL){
		$result =  dibi::query(
            'SELECT druh,id_komponenta,cena,jmeno FROM [vwKompSestavy]',
            '%if', isset($where), 'WHERE %and', isset($where) ? $where : array(), '%end',
            '%if', isset($order), 'ORDER BY %by', $order, '%end',
            '%if', isset($limit), 'LIMIT %i %end', $limit,
            '%if', isset($offset), 'OFFSET %i %end', $offset
