...
function format_number(nr,dec,decpt,sep) {

 if (!nr)
  return '0';
 if (typeof(dec)=='undefined')
  dec = 2;
 if (typeof(decpt)=='undefined')
  decpt = '.';
...
function price_reload() {
 ...
 def = $('side-config-detail-default');
 total += parse_int( (def.getElementsByTagName('span')[0].innerHTML) );
 
 $('side-config-detail').update();
 $('side-config-detail').insert(def);
 
 cont = $('config-cont').getElementsByTagName('input');
 for (i=0; i<cont.length; i++) {
  if (cont[i].checked) {
   price = cont[i].parentNode.getElementsByTagName('span');
   if (price.length == 2) {
    price_1 = parse_int(price[1].innerHTML);
    total += price_1;
    text = cont[i].parentNode.getElementsByTagName('label')[0].innerHTML;
    text = text.split('<')[0];
    text = text+': '+format_number(price_1, 0,'.',' ')+',- Kč';
    li = new Element('li');
    li.insert(text);
    $('side-config-detail').insert(li);
   }
  }
 }
 
 $('price-nahledNoDPHprice').update(format_number(total, 0,'.',' ')+',- Kč');
 $('price-nahledPrice').update(format_number(total*1.2, 0,'.',' ')+',- Kč');

 return;
}

function price_reload_init() {
 price_reload();
 return;
}
/*
window.onscroll = function() {
 ...
 if ( (scro+scsdf) > scsot )
  $('sidebar-config-float').style.paddingTop = ( (scro - scsot) + scsmo + scsdf)+'px';
 else
  $('sidebar-config-float').style.paddingTop = scsdf+'px';
}
*/